...
import controllers.cargarCategorias
...
fun menuInicio() {
    ...
    when (Integer.parseInt(opcion)) {
        ...
        3 -> consultaProductoPorCategoria(categorias)
...
fun consultaProductoPorCategoria(categorias: MutableMap<Int, Categoria>) {
    ...
    for ((key, value) in categorias) {
        ...
        if(categoriaBuscar in value.descripcion.lowercase(Locale.getDefault())){
            //  println("$key = $value")
            println("Nombre: ${value.descripcion}")
            // println("Se econtro el producto")
            println("************************")
            bandera =true
        }
